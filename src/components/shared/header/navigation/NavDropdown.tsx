...
export default function NavDropdown({
  ...
  const isChildrenNotEmpty = (children: DynamicPage["children"]) => {
    return children && children?.length && children?.length > 0 ? true : false;
  };
          ...
          >
            <div className="flex items-center gap-2">
              ...
              {isChildrenNotEmpty(item.children) && (
                ...
                >
                  <ShevronIcon
                    className={clsx("w-4 h-4 fill-white svg-shadow-white")}
                  />
                </button>
              )}
            </div>
            <AnimatePresence>
              {isDropdownOpen && isChildrenNotEmpty(item.children) && (
                <motion.ul
