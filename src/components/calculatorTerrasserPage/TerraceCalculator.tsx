      ...
      >
        {({ values, setFieldValue, setValues }) => {
          ...
          const materialSections: Array<{
            ...
            materialType && fieldsData[materialType as keyof typeof fieldsData]
              ? (fieldsData[materialType as keyof typeof fieldsData] as Array<{
                  sectionTitle: string;
                  id: string;
                  description?: string;
                  fields: Array<{
              ...
              <Container>
                ...
                >
                  <CalcSection
                    ...
                    fields={fieldsData.materialtype.map((field) => ({
                      id: field.id,
                      label: field.label,
                      value: field.value,
                      image: field.image,
                    ...
                    selectedValue={
                      typeof values.materialtype === "string"
                        ? values.materialtype
                        : String((values.materialtype as FieldData).value)
                    }
                    onChange={(id, value) => {
                      const materialField = fieldsData.materialtype.find(
                        (f) => f.value === value
                      );
                ...
                >
                  <AreaInput
                    value={
                      typeof values.area === "object"
                        ? Number((values.area as FieldData).value)
                        : Number(values.area)
                    }
                    onChange={(value) => {
                      setFieldValue("area", {
                    ...
                    return (
                      ...
                      >
                        <CalcSection
                          ...
                          selectedValue={
                            typeof values[section.id] === "object"
                              ? String((values[section.id] as FieldData).value)
                              : String(values[section.id])
                          }
                          onChange={(fieldId, value, label, category) => {
                  ...
                  {materialType && (
                    ...
                    >
                      <CalcSection
                        id="padding"
                        title="Bund"
                        fields={fieldsData.padding[0].fields.map((field) => ({
                          id: field.id,
                          label: field.label,
                          value: String(field.value),
                          image: {
                        ...
                        selectedValue={
                          typeof values.padding === "object"
                            ? String((values.padding as FieldData).value)
                ...
                {hasSelections && materialType && (
                  <>
                    ...
                    {total > 0 && <CalculatedPrice total={total} />}
                  </>
                )}
              </Container>
            </Form>
